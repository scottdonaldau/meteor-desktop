...
export default class MeteorApp {
    ...
    async ensureDesktopHCPPackages() {
        if (this.$.desktop.getSettings().desktopHCP) {
            ...
            const packages = ['omega:meteor-desktop-watcher', 'omega:meteor-desktop-bundler'];
            const packagesWithVersion = packages.map(packageName => `${packageName}@${this.$.getVersion()}`);

            try {
                await this.ensurePackages(packages, packagesWithVersion, 'desktopHCP');
            } catch (e) {
                throw new Error(e);
            }
        } else {
    ...
    checkMeteorVersion(versionRange) {
        let release = this.getMeteorRelease();
    ...
    chooseStrategy() {
        ...
        const release = this.getMeteorRelease().match(/(^\d+\.\d+\.\d+)/gmi)[0];
    ...
    changeDdpUrl() {
        ...
        this.log.info(this.$.env.options.ddpUrl);
